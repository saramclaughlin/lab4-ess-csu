...
cont_table_two <- table(salamander_clean$section, salamander_clean$unittype)
chisq.test(cont_table_two)
...
salamander_clean  |>  
  ...
  ggpubr::ggbarplot(x = 'unittype', y = 'n', 
                    ...
                    add = "mean_se") 
```
## The results of the Chi-squared test show a very low p-value (p-value < 2.2e-16) which indicates strong significance between forest type and channel unit with Coastal giant salamander abundance. 
...
clearc_weight <- salamander_clean |> 
  filter(section == "CC") |> 
  pull(weight_g)

oldg_weight <- salamander_clean |> 
  filter(section == "OG") |> 
  pull(weight_g)

var.test(clearc_weight, oldg_weight)

ggpubr::ggarrange(ggpubr::gghistogram(clearc_weight, main = "Clear Cut"), 
                  ggpubr::gghistogram(oldg_weight, main = "Old Growth"))

var.test(log(clearc_weight), log(oldg_weight))
t.test(log(salamander_clean$weight_g) ~ salamander_clean$section, var.equal = TRUE)

```
## The results of the final t-test shows a very small p-value (< 2.2e-16), indicating we should reject the null hypothesis. So, there is a significance between forest type and channel unit with Coastal giant salamander biomass. According to their means, Coastal giant salamander body size is significantly higher in clear cut forests than old growth forests.
...
```
## The results of this test indicate a highly significant positive relationship (rho = 0.992) between Cutthroat trout body length and body mass. 
...
### BOXPLOT STILL LOOKS EXACTLY THE SAME AS ORIGINAL !!!!

```{r}
site_order <- pie_crab |>
  group_by(site) |>
  summarize(mean_lats = mean(latitude, na.rm = TRUE)) |>
  arrange(mean_lats) |>
  pull(site)

pie_crab$site <- factor(pie_crab$site, levels = site_order)

ggboxplot(pie_crab, x = 'site', y= 'size', col = 'site') +
  geom_jitter(size =.25) +
  theme(legend.position = "none") +
  labs(
    x = "Site by Latitude",
    y = "Crab Size",
    title = "Crab Size by Site Ordered with Increasing Latitude"
  )
```
...
```
## This boxplot illustrates the correlation between crab size and site as a result of varying latitude, with sites ordered by increasing latitude. According to the boxplot, the mean crab size generally increases as mean latitude increases across sites. Thus, the plot provides evidence for the conclusion that crabs from higher-latitude sites will be larger on average than those from lower latitude sites.   
...
pie_crab |> 
  ggscatter(x = 'water_temp_sd', y = 'size', 
            ...
            add = "reg.line")
```
## The results of this linear model (Pr = 0.528 and Estimate = 0.09938) show that annual variation in water temperature does not have a significant influence or effect on crab size. 

```{r}
#| echo: false
#| out.width: "100%"
knitr::include_graphics('water_plot.png')
```
...
summary(crab_mlm)
```
## The correlation tables shows that air temperature deviation is highly, positively correlated with latitude.
## The multiple linear regression model reports a very small overall p-value (<2e-16), which suggests the combined predictor variables have a significant impact on crab size. According to individual p-values, latitude has the largest influence on crab size.
